...
describe('Create Classification Sheet (e2e)', () => {
  beforeAll(async () => {
  ...
  it ('POST /courses/:id/classification/sheet', async () => {
    const { token } = await makeAuth()

    const response = await request(app.server)
      .post(`/courses/${course.id.toValue()}/classification/sheet?hasBehavior=true`)
