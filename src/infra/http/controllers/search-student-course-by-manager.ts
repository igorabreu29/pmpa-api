...
export async function searchStudentCourseByManagerDetails(
  app: FastifyInstance
) {
  app
    ...
    .get('/manager/students/search', {
      ...
      return res.status(200).send({
        studentsByPole: students.map(student => ({
          students: student.studentsByPole.map(studentByPole => StudentCourseDetailsPresenter.toHTTP(studentByPole)),
          pages: student.pages,
