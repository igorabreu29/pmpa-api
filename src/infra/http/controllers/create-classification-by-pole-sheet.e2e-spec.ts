...
describe('Create Classification By Pole Sheet (e2e)', () => {
  beforeAll(async () => {
  ...
  it ('POST /courses/:id/poles/:poleId/classification/sheet', async () => {
    const { token } = await makeAuth()

    const response = await request(app.server)
      .post(`/courses/${course.id.toValue()}/poles/${pole.id}/classification/sheet?hasBehavior=true`)
