...
import { UpdateProfileUserUseCase } from "./update-profile-user.ts";
...
let sut: UpdateProfileUserUseCase

describe(('Profile Update User Use Case'), () => {
  beforeEach(() => {
    ...
    sut = new UpdateProfileUserUseCase(usersRepository)
  })

  it ('should not be able to update user if he does not exist', async () => {
    const result = await sut.execute({ userId: 'not-found' })
