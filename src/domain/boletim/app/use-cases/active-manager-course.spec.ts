...
import { ActiveManagerCourseUseCase } from "./active-manager-course.ts";
...
let sut: ActiveManagerCourseUseCase

describe('Active Manager Course Use Case', () => {
  beforeEach(() => {
    ...
    sut = new ActiveManagerCourseUseCase(
      managersRepository,
      coursesRepository,
      managersCoursesRepository
    )
  ...
  it ('should not be able to active manager course if access is manager or student', async () => {
  ...
  it ('should not be able to active manager course if course does not exist', async () => {
    const result = await sut.execute({ 
      id: '', 
      courseId: 'not-found', 
      reason: '',
  ...
  it ('should not be able to active manager course if manager does not exist', async () => {
  ...
  it ('should be able to active manager course', async () => {
    const course = makeCourse()
    coursesRepository.create(course)
    ...
    expect(managersCoursesRepository.items[0].isActive).toBe(true)
  })
})