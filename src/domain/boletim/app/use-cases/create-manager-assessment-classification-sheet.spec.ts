...
import { CreateManagerAssessmentClassificationSheetUseCase } from "./create-manager-assessment-classification-sheet.ts";
import { makeGetManagerAssessmentClassificationUseCase } from "test/factories/make-get-manager-assessment-classification-use-case.ts";
...
let getManagerAssessmentClassification: GetManagerAssessmentClassificationUseCase
let sheeter: FakeSheeter
let sut: CreateManagerAssessmentClassificationSheetUseCase

describe('Create Manager Assessment Classification Sheet', () => {
  beforeEach(() => {
    ...
    getManagerAssessmentClassification = makeGetManagerAssessmentClassificationUseCase({
      coursesRepository,
      managerCoursesRepository,
      studentCoursesRepository,
    ...
    sut = new CreateManagerAssessmentClassificationSheetUseCase(
      coursesRepository,
      managerCoursesRepository,
  ...
  it ('should be able to create course classification by manager sheet', async () => {
    ...
    expect(result.value).toMatchObject({
      filename: `${course.name.value} - Classificação Sem Comportamento.xlsx`
    })
  })
})