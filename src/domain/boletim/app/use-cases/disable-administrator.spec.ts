...
import { DisableAdministratorUseCase } from "./disable-administrator.ts";

let administratorsRepository: InMemoryAdministratorsRepository
let sut: DisableAdministratorUseCase

describe('Disable Administrator Use Case', () => {
  beforeEach(() => {
    administratorsRepository = new InMemoryAdministratorsRepository()
    sut = new DisableAdministratorUseCase(administratorsRepository)
  })
  ...
  it ('should not be able to active administrator', async () => {
    ...
    expect(administratorsRepository.items[0].isActive).toBe(false)
  })
})