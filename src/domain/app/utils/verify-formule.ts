...
export const formules = {
  period({ assessments, behaviorAverage }: FormuleProps) {
    ...
    const studentIsRecovering = assessments.some(({ isRecovering }) => isRecovering)
  ...
  module({ assessments, behaviorAverage }: FormuleProps) {
    const studentIsRecovering = assessments.some(({ isRecovering }) => isRecovering)
    
    const assessmentsAverage = assessments.reduce((previousAverageAssessment, currentAverageAssessment) => {
      return previousAverageAssessment + currentAverageAssessment.average 
