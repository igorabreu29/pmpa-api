...
import { OnManagerActivated } from './on-manager-activated.ts'
...
describe('On Manager Activated', () => {
  ...
  it ('should send a report when an manager is created', async () => {
    ...
    const managerCourse = makeManagerCourse({ courseId: course.id, managerId: manager.id, isActive: false })
