...
export class OnAssessmentBatchRemovedGrade implements EventHandler {
  constructor (
  ...
  setupSubscriptions(): void {
    DomainEvents.register(
      this.sendRemoveGradeAssessmentBatchReport.bind(this),
      AssessmentBatchEvent.name
    )
  }

  private async sendRemoveGradeAssessmentBatchReport({ assessmentBatch, reporterIp, ocurredAt }: AssessmentBatchEvent) {
    ...
    if (course && reporter) {
      await this.sendReportBatch.execute({
        title: 'Notas removidas em lote',
        content: `
          IP: ${reporterIp}
          Course: ${course.name.value}
          Remetente: ${reporter.username.value} (${reporter.role})
          Data: ${formattedDate}
          ${reporter.username.value} removeu notas em lote
        `,
