...
export class OnBehaviorBatchRemovedGrade implements EventHandler {
  constructor (
  ...
  setupSubscriptions(): void {
    DomainEvents.register(
      this.sendRemoveGradeBehaviorBatchReport.bind(this),
      BehaviorBatchEvent.name
    )
  }

  private async sendRemoveGradeBehaviorBatchReport({ behaviorBatch, reporterIp, ocurredAt }: BehaviorBatchEvent) {
    ...
    if (course && reporter) {
      await this.sendReportBatch.execute({
        title: 'Notas de comportamento removidas em lote',
        content: `
          IP: ${reporterIp}
          Curso: ${course.name.value}
          Remetente: ${reporter.username.value} (${reporter.role})
          Data: ${formattedDate}
          ${reporter.username.value} removeu notas de comportamento em lote
        `,
